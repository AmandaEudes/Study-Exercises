...
# numbers, and, as you can imagine, they have a lot of favorite numbers (like 0
# or 73 or 6174).
...
def binary_search(list: list[int], value: int) -> int:
    """Search for number in numbers via binary search.

    Args:
        list (list[int]): list of integer numbers
        value (int): value to search

    Raises:
        ValueError: error message if the number to be searched is not in the
        list

    Returns:
        (int): position in the list of the value to be searched
    """
